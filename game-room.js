document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderPlayers() {
        playersGrid.innerHTML = currentPlayers.map(player => `
            <div class="player-card p-4 rounded-lg flex flex-col items-center gap-3 ${player.alive ? '' : 'dead'} ${player.id === socket.id ? 'active' : ''}" data-id="${player.id}">
                <div class="w-16 h-16 rounded-full bg-gray-800 flex items-center justify-center text-2xl font-bold relative">
                    ${player.username.substring(0, 2).toUpperCase()}
                    ${player.id === socket.id ? '<span class="absolute -top-1 -right-1 bg-blood text-[10px] px-1 rounded">YOU</span>' : ''}
    ...
    chatForm.addEventListener('submit', (e) => {
        ...
        if (input.value.trim()) {
            socket.emit('send-chat', { roomCode, message: input.value.trim(), username });
            input.value = '';
        }
    });
